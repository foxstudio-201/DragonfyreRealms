...
app.setAppUserModelId('com.dragonfyrerealms.launcher')
...
function createMainWindow() {
  ...
  mainWindow = new BrowserWindow({
    width: 650, height: 890,
    minWidth: 650, minHeight: 890,
    maxWidth: 650, maxHeight: 890,
    resizable: false,
    maximizable: false,
    frame: false, transparent: false,
    show: false,
    backgroundColor: '#080808',
    title: 'Dragonfyre Realms',
    icon,
...
function createTray() {
  try {
    ...
    tray.setToolTip('Dragonfyre Realms')
    tray.setTitle('Dragonfyre Realms')
    ...
    const trayMenu = Menu.buildFromTemplate([
      {
        label: 'Dragonfyre Realms', enabled: false,
        ...(menuIcon ? { icon: menuIcon } : {}),
      },
      { type: 'separator' },
      {
        label: 'Mở Dragonfyre Realms',
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('app:backgroundPath', () => {
    const p = path.join(app.getPath('appData'), '.DragonfyreRealms', 'background-launcher.png')
  ...
  ipcMain.handle('server:status', async () => {
    try {
      ...
      return { server: { ip: SERVER_ADDRESS, name: 'Dragonfyre Realms Server' }, ...res }
    } catch (err) {
  ...
  try {
    const os = require('os')
    const updateDir = path.join(os.tmpdir(), 'DragonfyreRealms-update')
...
ipcMain.handle('fabric:getLoaderVersions', async (e, gameVersion) => {
  ...
  try {
    ...
    const data = await new Promise((resolve, reject) => {
      https.get(url, { headers: { 'User-Agent': 'DragonfyreRealms/1.0' } }, (res) => {
        let body = ''
