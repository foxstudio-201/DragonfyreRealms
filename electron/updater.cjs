...
const REPO = 'foxstudio-201/dragonfyrerealms'
let BUILTIN_TOKEN = ''
...
async function downloadUpdateToTemp(url, onProgress) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'dragonfyre-update-'))
  const dest = path.join(dir, 'dragonfyre-realms-setup.exe')
