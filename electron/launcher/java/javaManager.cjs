...
function httpsGetRaw(url) {
  return new Promise((resolve, reject) => {
    ...
    client.get(url, { headers: { 'User-Agent': 'DragonfyreRealms/1.0' } }, (res) => {
...
async function findJavaInstallations() {
  ...
  const runtimesDir = path.join(app.getPath('appData'), '.DragonfyreRealms', 'runtimes')
  ...
  return results.sort((a, b) => {
    const aManaged = a.path.includes('.DragonfyreRealms')
    const bManaged = b.path.includes('.DragonfyreRealms')
