...
const DEFAULT_REPO = 'Dattrinh123678/1.0'
...
function baseVersionFilePath(instancePath) {
  return path.join(instancePath, '.dragonfyrebase-version')
}
const OPTIONS_BACKUP_FILE = 'options.txt.dragonfyrebackup'
...
function httpGetJson(url) {
  return new Promise((resolve, reject) => {
    const headers = { 'User-Agent': 'Dragonfyre-Realms-Launcher', Accept: 'application/vnd.github+json' }
...
function httpHeadRedirect(url) {
  return new Promise((resolve, reject) => {
    const req = https.request(url, { method: 'HEAD', headers: { 'User-Agent': 'Dragonfyre-Realms-Launcher' } }, (res) => {
      res.resume()
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
...
function httpGetText(url) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, { headers: { 'User-Agent': 'Dragonfyre-Realms-Launcher' } }, (res) => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        res.resume()
...
function downloadFile(url, destPath, onProgress, signal) {
  return new Promise((resolve, reject) => {
    const headers = { 'User-Agent': 'Dragonfyre-Realms-Launcher', Accept: 'application/octet-stream' }
    if (TOKEN) headers.Authorization = `Bearer ${TOKEN}`
    const opts = { headers }
...
async function ensureDownloaded(url, destPath, onProgress, signal) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      console.error(`[dragonfyre-sync] Tải thất bại (lần ${attempt}/2): ${err.message}`)
      lastErr = err
    }
    try {
...
function readUpdateTxt(zipPath) {
  if (!/\.rar$/i.test(zipPath)) {
    try {
      ...
      console.error(`[dragonfyre-sync] Không đọc được update.txt trong zip: ${err.message}`)
      return null
    }
  }
...
async function moveContentsUp(base, sub) {
  ...
  function moveOne(s, d) {
    try {
      ...
      console.error(`[dragonfyre-sync] Dời ${s} → ${d}: ${err.message}`)
    }
  }
...
async function extractZipDirectly(zipPath, destDir, opts = {}) {
  ...
  for (const c of candidates) {
    try {
      ...
      console.error(`[dragonfyre-sync] ${c.tool} thất bại: ${err.message}`)
    }
  }
...
function dlCacheDir(instancePath, url) {
  ...
  return path.join(instancePath, '.dragonfyre-sync-cache', hash)
}
...
async function safeRm(dir) {
  ...
  for (let attempt = 1; attempt <= 4; attempt++) {
    try {
      ...
      if (attempt === 4) {
        console.error(`[dragonfyre-sync] Không dọn được ${dir}: ${err.message}`)
        return
      }
...
async function runBaseDataSync(profile, onProgress) {
  ...
  try {
    ...
    const release = await getWebRelease(REPO)
    ...
    fs.writeFileSync(baseVersionFilePath(instancePath), release.version, 'utf8')
    synced = true
...
async function checkBaseData(profile) {
  ...
  try {
    ...
    return {
      ok: true,
      installed: !!local,
      hasUpdate: local !== release.version,
      local,
...
async function runRepairDataSync(profile, onProgress) {
  ...
  try {
    ...
    return { ok: true, version: release.version, deletedCount }
  } catch (err) {
    if (err?.aborted) {
...
module.exports = { runBaseDataSync, checkBaseData, runRepairDataSync, downloadFile, updateOptionsBackup, optionsBackupInfo }
