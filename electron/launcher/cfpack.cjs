...
const CF_USER_AGENT = 'Dragonfyre-Realms-Launcher/0.2.20'
...
function packCacheDir(instancePath, fileId) {
  ...
  return path.join(instancePath, '.dragonfyre-sync-cache', 'cfpack-' + hash)
}
...
async function runCfPackInstall(profile, onProgress) {
  ...
  try {
    ...
    onProgress({ phase: 'mods', item: 'Tải mods', percent: 0, log: `Đang tải ${okMods.length} mods (8 file song song)...` })
    await mapLimit(okMods, 8, async (u) => {
      checkAbort()
