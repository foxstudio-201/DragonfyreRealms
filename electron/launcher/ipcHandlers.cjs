...
const { runBaseDataSync, checkBaseData, runRepairDataSync, updateOptionsBackup, optionsBackupInfo } = require('./dataSync.cjs')
...
const DATA_DIR      = path.join(app.getPath('appData'), '.DragonfyreRealms')
...
function registerLauncherHandlers(getTrustedWindow) {
  ...
  ipcMain.handle('dataSync:repair', async (e) => {
    ...
    try {
      ...
      console.error('[dragonfyre-sync] Lỗi sửa chữa:', err)
...
ipcMain.handle('dataSync:run', async (e) => {
    ...
    function send(data) {
      if (!win.isDestroyed()) win.webContents.send('dragonfyre-sync:progress', data)
    }
    try {
      return await runBaseDataSync(profile, send)
    } catch (err) {
      console.error('[dragonfyre-sync] Lỗi đồng bộ dữ liệu:', err)
      send({ phase: 'done', item: 'Lỗi', percent: 100, log: `Lỗi tải dữ liệu: ${err.message}` })
      return { ok: false, error: err.message, stack: err.stack || '' }
    }
  })
  ...
  ipcMain.handle('spiget:search', async (e, opts) => {
    ...
    try {
      const data = await new Promise((resolve, reject) => {
        ...
        https.get(url, { headers: { 'User-Agent': 'DragonfyreRealms/1.0' }, timeout: 8000 }, res => {
          let body = ''
