...
const FIXED_PROFILE_NAME = 'Dragonfyre Realms'
...
function normalizeSingleForgeProfile(data) {
  ...
  if (!Array.isArray(data.profiles)) data.profiles = []
  let profile = data.profiles.find(p => p && p.id && p.name === FIXED_PROFILE_NAME)
  let changed = false
  if (!profile) {
    profile = createDefaultProfile(FIXED_PROFILE_NAME)
    changed = true
  }
  if (profile.loader !== FIXED_LOADER) { profile.loader = FIXED_LOADER; changed = true }
  if (profile.gameVersion !== FIXED_GAME_VERSION) { profile.gameVersion = FIXED_GAME_VERSION; changed = true }
  if (!profile.loaderVersion || profile.loaderVersion !== FIXED_FORGE_VERSION) {
    profile.loaderVersion = FIXED_FORGE_VERSION
    changed = true
  }
