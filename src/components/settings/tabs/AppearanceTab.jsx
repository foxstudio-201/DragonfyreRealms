...
export default function AppearanceTab({ settings, onChange }) {
  ...
  const THEMES = [
    { id: 'default', labelKey: 'settings.launcher.themeDefault', descKey: 'settings.launcher.themeDefaultDesc',
      preview: 'linear-gradient(135deg,#1a1a24,#0c0c14)' },
  ]
  ...
  return (
    <div className="px-6 py-5 space-y-6">
      ...
      <Section title={t('settings.launcher.theme')}>
          ...
          {THEMES.map(th => (
            <button key={th.id}
              ...
              className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all duration-150 text-left
                ${theme === th.id ? 'border-violet-500/40 bg-violet-500/8' : 'border-white/5 bg-white/3 hover:bg-white/6 hover:border-white/12'}`}>
              <div className="w-12 h-12 rounded-lg border-2 flex-shrink-0"
                style={{ background: th.preview, borderColor: theme === th.id ? '#a78bfa' : 'rgba(255,255,255,0.12)' }}>
                <span className="block w-full h-full" />
              </div>
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-semibold ${theme === th.id ? 'text-violet-400' : 'text-white/80'}`}>{t(th.labelKey)}</p>
                <p className="text-xs text-white/30 mt-0.5 leading-relaxed">{t(th.descKey)}</p>
              </div>
              {theme === th.id && (
                <span className="w-5 h-5 rounded-full bg-violet-500/25 flex items-center justify-center flex-shrink-0">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="w-3 h-3 text-violet-300">
